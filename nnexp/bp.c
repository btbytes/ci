...
/**************************************************************/
 ...
 static char                   weight_file_name[MAX_NUM_CHARS];
 static char                   result_file_name[MAX_NUM_CHARS];
...
*
*************************************************************/
void BP_Input_From_PSO (float *vec)
{   int idx;
    int counter = 0;

	 for (idx = 0; idx< patset.dim_in; idx++)
	 {
		  patset.patterns[0][idx] = vec[counter++];
	 } 
}
...
*
*************************************************************/
float BP_Get_Criterion (void)
{
    return(bp.env.criterion);     
}

char* BP_Get_Result_File_Name(void)
{
	 return(result_file_name);
}

int   BP_Get_Target_Dimension(void)
{
	 return(patset.dim_out);
}

void  BP_Get_Target(float *target)
{
	 int idx;
	 for (idx = 0; idx < patset.dim_out; idx++)
	 {
        target[idx] = target_out[idx];
	 }
}
/************************************************************/
/*      BP  functons                                        */    
...
static void read_bp_parameters (char *dataFile)
{
    ...
    // open the runfile to input parameters
	 ...
	 // read BP weights
	 fscanf(frun, "%s", weight_file_name);
       // read BP's Arch
    ...
    // read in pattern parameters
    patset.size = 1;                 // number of pattern
    ...
    for (idx_i = 0 ; (idx_i < patset.dim_out) ; idx_i++)
	 {
		 fscanf(frun, "%f",&(target_out[idx_i]));
	 }

	 // read in error ternimation criterion
	 fscanf(frun, "%f",&(bp.env.criterion));

	 // read in result file name
    fscanf(frun, "%s", result_file_name);
...
static void print_net_parameters (void)
{
    int idx_i;
	 ...
	 printf( "number of patterns:    %d\n",(patset.size));                 // number of pattern
	  // display pattern input dimension
	 printf( "pattern input dimension:    %d\n",(patset.dim_in));               // input dimension
	  // display pattern output dimension
	 printf( "pattern output dimnesion:    %d\n",(patset.dim_out));              // output dimension
	  // display target output pattern
	 for (idx_i = 0 ; (idx_i < (patset.dim_out)) ; idx_i++)
	 {
			printf("target output[%d] = %f\t",idx_i,target_out[idx_i]);
	 }
	 printf("\n");
	 // display error termination criterion
	 printf("error termination criterion: %f\n", bp.env.criterion);

	 // display reuslt file name
...
static void update_recall_result(void)
{
	 ...
	 {   // loop throught the neurons of the output layer
		  temp = bp.arch.layers[bp.arch.size - 1].neurons[idx_out].out - target_out[idx_out];
		  bp.mse += (temp * temp);
	 }
}
...
static void bp_initialize_weights (void)
{
    ...
    {   // RECALL operation mode
        ...
        // open file for write
        if ((fout=fopen(weight_file_name,"r"))==NULL)
	      {
		      printf("Cant open file for read BP training  results");
		      exit(1);
	      }
...
static void bp_get_pattern (void)
{
	...
	}*/  // target is fixed for explanation facility
